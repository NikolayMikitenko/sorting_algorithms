...
#numbers = random.randint(1000000, size=(1000000))
numbers = random.randint(s, size=(s))
unique = []
for i in numbers:
    if i not in unique:
        unique.append(i)
...
#for i in range(1000):
for i in range(ln):    
    num = unique[i*100-100:i*100]
    start = time.time()
    for n in num:
        root = tree.insert_node(root = root, key = n)
    end = time.time()
    print(f"{i*100+100},{(end-start)*1000},{tracemalloc.get_traced_memory()[1]}")
tracemalloc.stop()
...
for i in range(ln):
    ...
    start = time.time()
    for n in num:
        tree.get_node(root=root, key = n)
    end = time.time()
    get = (end-start)*1000
    start = time.time()
    for n in num:
        root = tree.delete_node(root = root, key = n)
    end = time.time()
    delete = (end-start)*1000
    print(f"{su-i*100},{get},{delete}")
    for n in num:
        unique.pop(0)



# df = pd.DataFrame([[0, 0, 0]], columns=['size, rows', 'duration, sec', 'duration per item, microsec'])

# rdq = 100
# rds = [100, 1000, 10000, 100000] # , 1000000] #, 10000000] #, 10000000, 100000000, 1000000000]
...
#     df = pd.concat([df, pd.DataFrame([[rds[size], sd, sdi]], columns=['size, rows', 'duration, sec', 'duration per item, microsec'])])
